...
DIST_GE = [0, 0.035, 0.087, 0.237, 0.393, 
           0.529, 0.664, 0.787, 0.862, 0.95, 1.0]
...
def get_groups(scores, df_post):
    ...
    Returns:
        df_post con columnas adicionales: prob, prob_frescura, puntuacion_tlv, 
        grupo_ejec_tlv.
    """
    df_post["prob"] = scores
    
    # Calculo de prob_frescura
    df_post["prob_frescura"] = np.where(
        ...
        np.where(df_post["grp_campecs06m"] == "G4", 0.008, 
                 0.004))))
    
    # Rellenar nulos en prob_value_contact
    df_post["prob_value_contact"] = df_post["prob_value_contact"].fillna(0.000001)
    
    
    df_post["puntuacion_tlv"] = (
        df_post["prob"] * df_post["prob_value_contact"] * np.log(df_post["monto"] + 1) * df_post["prob_frescura"]
    )
    
    
    df_post["grupo_ejec_tlv"] = pd.qcut(
        df_post["puntuacion_tlv"], q=DIST_GE,
...
def save_replica(df_post, table, partition, 
    ...
    Columnas: codmes | tipdoc | coddoc | puntuacion | 
    modelo | fec_replica | grupo_ejec | score | orden | variable1 
    | variable2 | variable3
    """
